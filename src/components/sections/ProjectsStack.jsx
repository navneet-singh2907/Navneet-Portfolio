...
const HEADER_H = 72  // px — height of each sticky header bar
const NAV_OFFSET = 80 // px — clear the fixed navbar
...
const PROJECTS = [
  {
    ...
    gradient: 'linear-gradient(135deg, #0d1f12, #1a4a2e)',
    icon:  '🛡️',
  ...
  },
  {
    ...
    desc:  'Fully responsive personal portfolio built with React and Three.js. Features WebGL particle animations, smooth scrolling, and dynamic project sections.',
    tags:  ['React', 'Three.js', 'WebGL', 'CSS'],
...
export default function ProjectsStack() {
  return (
    <section id="projects" className="pstack-section">
      <div className="pstack-heading-wrap">
      ...
      <div className="pstack-list">
        {PROJECTS.map((p, i) => (
          <div
            key={p.num}
            className="pstack-scroll-section"
          >
            <div
              className="pstack-card"
              style={{ top: `${NAV_OFFSET + i * HEADER_H}px`, zIndex: i + 1 }}
            >
              {/* ── sticky header bar ── */}
              <div className="pstack-header">
                <div className="pstack-num-block">
                  <span className="pstack-num">{p.num}</span>
                  <span className="pstack-title">{p.title}</span>
                </div>
                <div className="pstack-header-actions">
                  {p.demo && (
                    <a href={p.demo} target="_blank" rel="noreferrer" className="pstack-live-btn">
                      LIVE PROJECT ↗
                    </a>
                  )}
                  {p.code && (
                    <a href={p.code} target="_blank" rel="noreferrer" className="pstack-code-btn" title="View code">
              ...
              {/* ── card body ── */}
              <div className="pstack-body">
                <div className="pstack-visual" style={{ background: p.gradient }}>
                  <span className="pstack-icon">{p.icon}</span>
                </div>
                <div className="pstack-info">
                  <p className="pstack-desc">{p.desc}</p>
